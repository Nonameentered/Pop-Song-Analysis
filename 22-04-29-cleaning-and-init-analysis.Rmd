...
suspects <- (cleaned_df %>% 
               filter(forcast > 12) %>% 
               arrange(forcast) %>%
               select(artist, track, cleaned_lyrics, forcast))
suspects
for (row in 1:nrow(suspects)) {
  print(paste(suspects[row, "track"], " - ", suspects[row, "artist"], suspects[row, "forcast"]))
...
```{r}
plot(log_listeners ~ year, main="", xlab="", ylab = "", cex.main = 1, cex.lab = 1)
```
