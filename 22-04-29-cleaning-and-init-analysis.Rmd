...
```{r}
qqPlot(forcast, xlab = "Normal Quantiles", ylab = "FORCAST readibility scores", main = "Normal Quantile Plot of FORCAST Readibility Scores")
```
```{r}
...
test1
```
*Note: keep the transformation in mind when interpreting (female artists have less complex songs)*
...
trans1 <- boxCox(lm(last_fm_listeners ~ gender + year + rank + region + artist_appearances + gender*year + year*rank + region*gender, data = subset_df))
